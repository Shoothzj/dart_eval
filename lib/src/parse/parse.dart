...
class Parse {
  ...
  ScopeWrapper parse(String content) {
    ...
    final tlUnit =
        EvalCompilationUnit([...dd, ...dartCore, ..._additionalDefines]);
    return ScopeWrapper(tlUnit.buildScope());
  }

  bool _isNotNull(dynamic it) => it == null ? false : true;

  DartDeclaration _parseDeclaration(
      ParseContext context, Declaration declaration) {
    if (declaration is ClassDeclaration) {
      ...
      return _parseVariableDecList(
          context, declaration as VariableDeclarationList, false, false);
    } else if (declaration is FieldDeclaration) {
      return _parseVariableDecList(context, declaration.fields,
          declaration.isStatic, _isNotNull(declaration.covariantKeyword));
    } else if (declaration is MethodDeclaration) {
      return _parseMethodDec(context, declaration);
    } else if (declaration is FunctionDeclaration) {
      return DartFunctionDeclaration(declaration.name.name,
          _parseFunctionExpression(context, declaration.functionExpression),
      ...
      return DartConstructorDeclaration(declaration.name?.name ?? '',
          _parseFPL(context, declaration.parameters));
    }
    throw ArgumentError('Unknown declaration type ${declaration.runtimeType}');
  }

  DartVariableDeclarationList _parseVariableDecList(ParseContext context,
      VariableDeclarationList list, bool isStatic, bool isCovariant) {
  ...
  // TODO getters
  DartMethodDeclaration _parseMethodDec(
      ParseContext context, MethodDeclaration declaration) {
    final mb = _parseFunctionBody(context, declaration.body);
    return DartMethodDeclaration(declaration.name.name, mb,
        _parseFPL(context, declaration.parameters!), declaration.isStatic);
  }

  List<ParameterDefinition> _parseFPL(
      ParseContext context, FormalParameterList list) {
    final lis = <ParameterDefinition>[];
    for (final param in list.parameters) {
      final nfp = (param is DefaultFormalParameter)
          ? param.parameter
      ...
      if (nfp is SimpleFormalParameter) {
        ...
        throw UnimplementedError(
            'Function typed formal parameters not yet supported');
      }
    ...
    if (statement is Block) {
      ...
      return DartReturnStatement(
          ...
          statement.expression == null
              ...
              : _parseExpression(context, statement.expression!));
    } else if (statement is IfStatement) {
      return DartIfStatement(
          statement.offset,
          statement.length,
          _parseExpression(context, statement.condition),
          _parseStatement(context, statement.thenStatement),
          statement.elseStatement == null
              ? null
              : _parseStatement(context, statement.elseStatement!));
    } else if (statement is ForStatement &&
        statement.forLoopParts is ForParts) {
      final parts = statement.forLoopParts as ForParts;
      return DartForStatement(
          statement.offset,
          statement.length,
          parts is ForPartsWithDeclarations
              ? _parseVariableDecList(context, parts.variables, false, false)
              : null,
          parts is ForPartsWithExpression && parts.initialization != null
              ? _parseExpression(context, parts.initialization!)
              : null,
          parts.condition != null
              ? _parseExpression(context, parts.condition!)
              : null,
          parts.updaters.map((u) => _parseExpression(context, u)).toList(),
          _parseStatement(context, statement.body));
    }
    throw ArgumentError(
        'Unknown statement type ${statement.runtimeType} or unknown variant');
  }

  DartBlockStatement _parseBlock(ParseContext context, Block block) =>
      DartBlockStatement(block.offset, block.length,
          block.statements.map((s) => _parseStatement(context, s)).toList());

  DartExpressionStatement _parseExpressionStatement(
          ParseContext context, ExpressionStatement statement) =>
      DartExpressionStatement(statement.offset, statement.length,
          _parseExpression(context, statement.expression));

  DartVariableDeclarationStatement _parseVariableDecStatement(
          ParseContext context, VariableDeclarationStatement statement) =>
      DartVariableDeclarationStatement(statement.offset, statement.length,
          _parseVariableDecList(context, statement.variables, false, false));

  DartVariableDeclaration _parseVariableDec(
      ParseContext context, VariableDeclaration declaration) {
    return DartVariableDeclaration(
        declaration.name.name,
        declaration.initializer == null
            ? null
            : _parseExpression(context, declaration.initializer!),
        isLate: declaration.isLate);
  }

  EvalExpression _parseExpression(ParseContext context, Expression expression) {
    if (expression is SimpleIdentifier) {
      return EvalIdentifierExpression(
          expression.offset, expression.length, expression.name);
      ...
      return EvalCallExpression(
          ...
          expression.target != null
              ? _parseExpression(context, expression.target!)
              : null,
          expression.methodName.name,
          expression.argumentList.arguments
              .map((m) => _parseExpression(context, m))
              .toList());
      ...
      return EvalPropertyAccessExpression(
          expression.offset,
          expression.length,
          _parseExpression(context, expression.realTarget),
          expression.propertyName.name);
    } else if (expression is AssignmentExpression) {
      return EvalAssignmentExpression(
          expression.offset,
          expression.length,
          _parseExpression(context, expression.leftHandSide)
              as EvalReferenceExpression,
      ...
      return EvalPostfixExpression(
          expression.offset,
          expression.length,
          _parseExpression(context, expression.operand)
              as EvalReferenceExpression,
          expression.operator.type);
    }
    throw ArgumentError(
        'Unknown expression found while parsing: ${expression.runtimeType}');
  }

  EvalFunctionExpression _parseFunctionExpression(
          ParseContext context, FunctionExpression expression) =>
      EvalFunctionExpression(
          expression.offset,
          expression.length,
          _parseFunctionBody(context, expression.body),
          _parseFPL(context, expression.parameters!));

  EvalIdentifierExpression _parseIdentifier(Identifier identifier) {
    if (identifier is SimpleIdentifier) {
      return EvalIdentifierExpression(
          identifier.offset, identifier.length, identifier.name);
    } else if (identifier is PrefixedIdentifier) {
      return EvalPrefixedIdentifierExpression(identifier.offset,
          identifier.length, identifier.prefix.name, identifier.name);
    }
  ...
  EvalExpression _parseLiteral(ParseContext context, Expression literal) {
    if (literal is StringLiteral) {
      ...
      throw ArgumentError(
          'Unknown literal found while parsing: ${literal.runtimeType}');
      ...
      return EvalListLiteral(
          ...
          literal.length,
          literal.elements
              .map((e) => _parseCollectionElement(context, e))
              .toList());
    } else if (literal is SetOrMapLiteral) {
      return EvalMapLiteral(
          literal.offset,
          literal.length,
          literal.elements
              .map((e) => _parseCollectionElement(context, e))
  ...
  EvalCollectionElement _parseCollectionElement(
      ParseContext context, CollectionElement element) {
    if (element is Expression) {
      return _parseExpression(context, element);
    } else if (element is MapLiteralEntry) {
      return EvalMapLiteralEntry(
          element.length,
          element.offset,
          _parseExpression(context, element.key),
          _parseExpression(context, element.value));
    }
    throw ArgumentError(
        'Unknown collection element found while parsing: ${element.runtimeType}');
  }

  EvalGenericsList _parseTypeParameterList(
      ParseContext context, TypeParameterList? list) {
    if (list == null) {
      return EvalGenericsList([]);
    }
    return EvalGenericsList(list.typeParameters
        .map((e) => _parseTypeParameter(context, e))
        .toList());
  }

  EvalGenericParam _parseTypeParameter(
      ParseContext context, TypeParameter param) {
